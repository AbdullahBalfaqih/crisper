...
// Use a global variable to store the connection pool to prevent re-creating it on every hot-reload in development.
...
// Add a function to execute the schema setup
export async function setupDatabase() {
    ...
    try {
        await client.query(`
            ...
            BEGIN
                ...
                IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'order_status') THEN
                    CREATE TYPE order_status AS ENUM ('new', 'preparing', 'ready', 'out-for-delivery', 'completed', 'rejected');
                END IF;
        ...
        await client.query(`
            ...
            BEGIN
                IF NOT EXISTS (
                    ...
                    WHERE table_name='users' AND column_name='password_reset_requests'
                ) THEN
                    ALTER TABLE users ADD COLUMN password_reset_requests JSONB;
                END IF;
            END$$;
        `);
        ...
        if (userCount[0].count === '0') {
          await client.query(`
            INSERT INTO users (full_name, username, email, phone_number, password_hash, role) VALUES
            ('مدير النظام', 'admin', 'admin@example.com', 'admin', 'admin', 'system_admin');
          `);
        }
