...
import { ArrowRight, Ticket, Upload, Banknote, ShieldCheck, Copy, X, Hourglass, ShoppingBag } from 'lucide-react';
...
export default function CheckoutPage() {
    ...
    const [orderStatus, setOrderStatus] = useState<'idle' | 'submitted'>('idle');
    const [banks, setBanks] = useState<Bank[]>([]);
    const [coupons, setCoupons] = useState<Coupon[]>([]);

    // Coupon lockout state
        ...
        const fetchInitialData = async () => {
            try {
                const [banksRes, couponsRes] = await Promise.all([
                    fetch('/api/banks'),
                    fetch('/api/coupons') // Assuming you have a coupons API
                ]);
    ...
    const handleConfirmOrder = () => {
        ...
        // In a real app, you would clear the cart after a successful API call
        clearCart();
          ...
          <CardContent className="space-y-3">
            ...
            {discount > 0 && (
                 <div className="flex justify-between items-center text-green-600 animate-in fade-in">
                    <span>الخصم</span>
                    <span className="font-mono">-{discount.toFixed(2)} ر.ي</span>
                </div>
            )}
             <div className="border-t border-dashed my-2"></div>
             <div className="flex justify-between items-center text-xl font-bold text-primary">
                <span>الإجمالي للدفع</span>
                <span className="font-mono">{finalTotal.toFixed(2)} ر.ي</span>
            </div>
          </CardContent>
        </Card>
